...
function uploadImg() {
  ...
  if (isSuc) {
    setLog("手机截图删除结果：" + ((files.remove(fileName) ? "成功" : "失败")), true);
...
function getReslt() {
  ...
  try {
    ...
    } else if (myStr === "上班打卡" && (textContains("下班打卡").exists() || descContains("下班打卡").exists())) {
      // 打开App时触发了自动打卡
      setLog("普通识别结果：" + myStr + "成功!");
    } else {
...
function pushMsg(msg) {
  let url = "https://pushdeer.ftqq.com/message/push?pushkey=" + pushKey + "&text=" + msg;
  http.get(url);
}
